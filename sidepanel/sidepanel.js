...
function toggleLanguage() {
    ...
    applyTranslations();
    // Re-render auth banner with current language if logged in
    if (isLoggedIn) {
        chrome.storage.local.get('user').then(({ user }) => {
            if (user) updateBannerForAuth(user);
        });
    }
    // Update Saved Jobs trigger button label
    const trigger = document.getElementById('jobHistoryTrigger');
    if (trigger) {
...
function updateScanBadge(remaining, total = MAX_SCANS) {
    ...
    // BUG 1 FIX: always force '0' string when count is zero so stale DOM values cannot persist
    if (count === 0) {
    ...
    if (count === 0) {
        ...
        els.guestBanner.classList.add('warn');
    }
}
...
function updateBannerForAuth(profile) {
    ...
    const creditLabel = (totalCredits === null && directRemaining === null)
        ...
        : `${packageName} — ${remaining ?? 0} ${currentLang === 'fr' ? 'crédits restants' : 'credits left'}`;
...
async function loadActiveJobsDropdown() {
    ...
    // Update trigger label with current language
    const triggerLabel = freshTrigger.querySelector('.sp-jobs-trigger-label');
    if (triggerLabel) triggerLabel.textContent = t('savedJobs');
...
function resetView() {
    // BUG 3 FIX: ensure these run first before any other state changes
...
async function init() {
        ...
        if (statusResp?.success) {
            if (statusResp.data.isLoggedIn) {
                ...
                if (credits_exhausted) {
                    // BUG 1 FIX: call updateScanBadge(0) to ensure consistent UI state,
                    // then override scanCountDisplay and add danger class explicitly
                    updateScanBadge(0);
                    els.scanCountDisplay.textContent = '0';
                    els.guestBanner.classList.remove('warn');
                    els.guestBanner.classList.add('danger');
    ...
    // BUG 3 FIX: stop event bubbling to prevent upgrade redirect being triggered
